...
--[[ local function displayNUIText(text)
	...
	if not src then 
		if src == playerPed then 
			origin = playerCoords 
		else origin = NetworkGetPlayerCoords(src) 
		end
	end
				...
				while isLoggedIn do
					...
					elseif paused then
						...
						if closestDoor.distance < (closestDoor.data.distance or closestDoor.data.maxDistance) then
							...
							if displayText ~= "" and (closestDoor.data.hideLabel == nil or not closestDoor.data.hideLabel) then lib.showTextUI(displayText) end
						else
							lib.hideTextUI()
							break
						end
