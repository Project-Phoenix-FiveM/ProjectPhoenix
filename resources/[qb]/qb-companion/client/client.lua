...
function ActivePed:remove(index)
    ...
    TriggerServerEvent('qb-companion:server:ForceRemoveNetEntity', netId)
    self.data[index] = nil
...
function ActivePed:removeAll()
    ...
    for key, value in pairs(ActivePed:petsList()) do
        ...
        TriggerServerEvent('qb-companion:server:updateAllowedInfo', currentItem, {
            key = 'XP',
        })
    end
    TriggerServerEvent('qb-companion:server:onPlayerUnload', tmpHash)
    self.data = {}
    self.onControl = -1
end
...
RegisterNetEvent('qb-companion:client:callCompanion')
AddEventHandler('qb-companion:client:callCompanion', function(modelName, hostileTowardPlayer, item)
    ...
    CoreName.Functions.Progressbar("callCompanion", "Calling companion", Config.Settings.callCompanionDuration * 1000,
        ...
        QBCore.Functions.TriggerCallback('qb-companion:server:updatePedData', function(result)
            ...
            if petData.hostile == true then
                TriggerServerEvent('qb-companion:server:despwan_not_owned_pet', petData.itemData.info.hash)
                return
            end
...
function request_healing_process(ped, item, process_type)
    ...
    CoreName.Functions.Progressbar("reviveing", "Reviveing",
        ...
        TriggerServerEvent('qb-companion:server:revivePet', current_pet, process_type)
        TaskFollowTargetedPlayer(ped, plyID, false)
    end)
end

RegisterNetEvent('qb-companion:client:update_health_value', function(item, amount)
    ...
    CreateThread(function()
        ...
        while DoesEntityExist(ped) and fninished == false do
            ...
            if IsPedDeadOrDying(savedData.entity) == false and savedData.maxHealth ~= currentHealth and
                ...
                currentHealth then
                -- ped is still alive
                TriggerServerEvent('qb-companion:server:updateAllowedInfo', {
...
RegisterNetEvent('qb-companion:client:forceKill', function(hash, reason)
...
RegisterNetEvent('qb-companion:client:despawn')
AddEventHandler('qb-companion:client:despawn', function(item, revive)
    if revive ~= nil and revive == true then
        ...
        TriggerServerEvent('qb-companion:server:setAsDespawned', item)
        return
    end
    local plyPed = PlayerPedId()
...
RegisterNetEvent('qb-companion:client:start_feeding_animation', function()
    ...
    CoreName.Functions.Progressbar("feeding", "Feeding", Config.core_items.food.settings.duration * 1000, false, false,
        ...
        TriggerServerEvent('qb-companion:server:increaseFood', current_pet.itemData)
    end)
end)

RegisterNetEvent('qb-companion:client:', function()
...
function start_drinking_animation()
    ...
    CoreName.Functions.Progressbar("pet_drinking", "drinking", Config.core_items.waterbottle.settings.duration * 1000,
        ...
        QBCore.Functions.TriggerCallback('qb-companion:server:decrease_thirst', function(result)

        end, current_pet.itemData)
    end)
end

RegisterNetEvent('qb-companion:client:filling_animation', function(item)
    CoreName.Functions.Progressbar("filling_animation", "filling bottle",
        ...
        TriggerServerEvent('qb-companion:server:filling_event', item)
    end)
end)

RegisterNetEvent('qb-companion:client:rename_name_tag', function(item)
    ...
    if name then
        ...
        TriggerServerEvent('qb-companion:server:rename_name_tag', name.pet_name)
    end
end)



RegisterNetEvent('qb-companion:client:rename_name_tagAction', function(name)
    ...
    CoreName.Functions.Progressbar("waitingForName", "waiting for Name",
        ...
        QBCore.Functions.TriggerCallback('qb-companion:server:renamePet', function(result)
...
RegisterNetEvent('qb-companion:client:collar_process', function()
    ...
    if inputData then
        ...
        CoreName.Functions.Progressbar("waitingForOwenership", "waiting for new owner",
            ...
            function()
                ...
                QBCore.Functions.TriggerCallback('qb-companion:server:collar_change_owenership', function(result)
