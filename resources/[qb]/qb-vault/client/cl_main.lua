...
--Blip for vault, If you create more add more here.

Citizen.CreateThread(function()
    for k, _ in pairs (Config.blips) do 
        if Config.blips[k].shopType == "vault" then
            local vaultArea = AddBlipForCoord(Config.blips[k].coords)
            SetBlipSprite(vaultArea, 255) 
            SetBlipColour(vaultArea, 5)
            SetBlipScale (vaultArea, 0.6)
            SetBlipAsShortRange(vaultArea, true)
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Storage Container")
            EndTextCommandSetBlipName(vaultArea)
        end
    end
end)
