...
function GiveMoney(player, money)
	...
	if Player then
		Player.Functions.AddItem('casinochips', money, false)
		TriggerClientEvent('inventory:client:ItemBox', Player.PlayerData.source, QBCore.Shared.Items["casinochips"], "add", money)
		TriggerEvent("qb-log:server:CreateLog", 'gamble', "Blackjack", "green", "**"..Player.PlayerData.name .. "** (citizenid: *" .. Player.PlayerData.citizenid .. "* | id: *(" .. Player.PlayerData.source .. "))*: has received " .. money .. "$")  
	end
...
function CheckPlayerBet(i, bet)
	...
	if playerChips then
		if playerChips.amount >= bet then
			Player.Functions.RemoveItem('casinochips', bet, false)
			TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["casinochips"], "remove", bet)
			TriggerEvent("qb-log:server:CreateLog", 'gamble', "Blackjack", "red", "**"..Player.PlayerData.name .. "** (citizenid: *" .. Player.PlayerData.citizenid .. "* | id: *(" .. Player.PlayerData.source .. "))*: has placed a bet of " .. bet .. "$")  
			canBet = true
		end
	end
										...
										while v.player_in == true and #v.hand < 5 do
											...
											if tableTracker[tostring(v.player)] == nil then
												...
												elseif move == "split" then
													...
													v.splitHand[#v.splitHand+1] = splitCard
													...
													v.splitHand[#v.splitHand+1] = card
													Wait(1500)
													DebugPrint("TABLE "..index..": DEALT "..GetPlayerName(v.player):upper().." "..card)
...
function PlayerSatDown(i, seat)
	...
	-- index = i
	-- chair = seat
	players[i][#players[i]+1] = {player = source, seat = seat, hand = {}, player_in = true, bet = 0}
