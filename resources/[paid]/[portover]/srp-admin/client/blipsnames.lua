...
Citizen.CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('srp-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('srp-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        SRPCore.Functions.Notify("Blips activated", "success")
    else
        ShowBlips = false
        SRPCore.Functions.Notify("Blips deactivated", "error")
    end
end)

RegisterNetEvent('srp-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        SRPCore.Functions.Notify("Names activated", "success")
    else
        ShowNames = false
        SRPCore.Functions.Notify("Names deactivated", "error")
    end
end)

RegisterNetEvent('srp-admin:client:Show', function(players)
	if PlayersBlips then
    ...
    for k, player in pairs(players) do
		...
		if not DoesBlipExist(blip) then
			if NetworkIsPlayerActive(player.id) then
			...
			if ShowBlips then
				if not DoesBlipExist(blip) then
					...
					table.insert(PlayersBlips, blip)
				end
			else
				RemoveBlip(blip)
				NetCheck1 = false
